...
import static com.foudroyantfactotum.mod.fousarchive.blocks.Structure.TEStructure.*;
...
public final class TEStructureShape extends FCAA_TE implements ITEStructure, ISidedInventory, IFluidHandler
{
    ...
    private Optional<TEStructure> originTE = Optional.absent();
    private boolean hasNotAttemptedAcquisitionOfOriginTE = true;

    private TEStructure getOriginTE()
    {
        return originTE.get();
    }
    ...
    private boolean hasOriginTE()
    {
        ...
        if (hasNotAttemptedAcquisitionOfOriginTE && te instanceof TEStructure)
        {
            if (!te.isInvalid())
            {
                originTE = Optional.of((TEStructure) te);
                return true;
            }
        }
    ...
    @Override
    public BlockStructure getMasterBlockInstance()
    {
    ...
    public BlockPos getMasterBlockLocation()
    {
        if (!masterLocation.isPresent())
        {
            ...
            final BlockStructure sb = StructureRegistry.getStructureBlock(definitionHash);
            ...
            masterLocation = Optional.of(localToGlobal(
                    ...
                    state.getValue(FACING), state.getValue(BlockStructure.MIRROR),
    ...
    {
        BlockStructure sb = StructureRegistry.getStructureBlock(definitionHash);

        if (sb != null)
        {
            final IBlockState state = worldObj.getBlockState(pos);

            if (state != null && state.getBlock() instanceof BlockStructureShape)
            {
                ...
                return block == null ?
                        ...
                        localToGlobal(
                                ...
                                state.getValue(BlockStructure.MIRROR)
                        );
            }
        }
